...
let artworkLink = new OPDSArtworkLink(<OPDSArtworkLink>{
  href: artworkUrl,
  rel: 'http://opds-spec.org/image',
});

let entry = new OPDSEntry(<OPDSEntry>{
...
let acquisitionFeed = new AcquisitionFeed(<AcquisitionFeed>{
  id: 'feed.xml',
  title: 'Feed',
  entries: [entry],
});

let navigationLink = new OPDSLink(<OPDSLink>{
  rel: 'subsection',
  title: 'title',
  href: 'href',
});

let linkEntry = new OPDSEntry(<OPDSEntry>{
  id: 'feed.xml',
  title: 'Feed',
  links: [navigationLink],
});

let navigationFeed = new NavigationFeed(<NavigationFeed>{
  id: 'feed.xml',
  title: 'Feed',
  entries: [linkEntry],
});
